...
Вход в витрину использует GitHub OAuth Device Flow. Сборка должна получить публичный client ID OAuth App через `GITHUB_OAUTH_CLIENT_ID` или `CANVASTTY_GITHUB_CLIENT_ID`; client secret в приложение не встраивается и не требуется. Без client ID вход в витрину недоступен, но проверка и установка по прямой ссылке продолжают работать. Отключение удаляет локальную зашифрованную сессию; при необходимости отдельно отзовите доступ в [настройках приложений GitHub](https://github.com/settings/applications).
